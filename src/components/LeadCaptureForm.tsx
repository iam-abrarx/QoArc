...
Assets/Links: ${formData.assets || 'None'}
          ...
          <div className="space-y-6 md:space-y-8">
            ...
            <div className="flex justify-center">
              <button 
                ...
                className="bg-primary text-white px-12 py-4 rounded-none font-bold uppercase tracking-widest text-[10px] shadow-premium hover:scale-105 transition-all disabled:opacity-50 disabled:grayscale cursor-pointer"
              >
        ...
        return (
          <div className="max-w-2xl mx-auto space-y-6 md:space-y-8">
            <div className="relative">
              <LinkIcon className="absolute left-5 top-1/2 -translate-y-1/2 text-primary/30" size={16} />
              <input 
                type="text" 
                placeholder="Paste Google Drive or Dropbox link"
                value={formData.assets}
                onChange={e => setFormData({...formData, assets: e.target.value})}
                className="w-full bg-white border border-outline-variant/10 rounded-none p-4 pl-14 text-sm focus:outline-none focus:border-primary/40 transition-all font-sans italic"
              />
            </div>
            <div className="text-center text-[10px] font-bold text-primary/20 uppercase tracking-widest">— OR UPLOAD DIRECTLY —</div>
            <div className="w-full py-8 px-4 bg-white border-[0.5px] border-dashed border-primary/20 rounded-none flex flex-col items-center justify-center space-y-4 group hover:border-primary/40 transition-all cursor-pointer">
              <div className="w-10 h-10 rounded-none bg-surface-container-low flex items-center justify-center text-primary/40 group-hover:bg-primary group-hover:text-white transition-all shadow-sharp">
                <Upload size={16} />
              </div>
            ...
            <div className="flex justify-center">
               <button 
                onClick={nextStep}
                className="text-primary/60 hover:text-primary px-8 py-3 font-bold uppercase tracking-widest text-[10px] transition-all cursor-pointer"
              >
                Skip this step →
              </button>
            </div>
          ...
          <div className="max-w-3xl mx-auto space-y-4 md:space-y-5">
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="w-full bg-white border border-outline-variant/10 rounded-none p-4 pl-16 text-sm focus:outline-none focus:border-primary/40 transition-all font-sans"
              />
            </div>
            <div className="pt-4 md:pt-6 flex justify-center">
               <button 
                ...
                className="bg-primary text-white px-12 py-4 rounded-none font-bold uppercase tracking-widest text-xs shadow-premium hover:scale-105 transition-all disabled:opacity-50 disabled:grayscale flex items-center gap-4 cursor-pointer"
              >
  ...
  return (
    <div className="relative w-full max-w-screen-xl mx-auto py-6 md:py-10 px-4">
      {/* Progress Header */}
      <div className="flex justify-between items-center mb-8 md:mb-12 px-8">
      ...
      <div className="min-h-[350px] md:min-h-[400px] flex flex-col justify-center">
        <AnimatePresence mode="wait">
          {!isSubmitted ? (
            ...
            >
              <h2 className="text-3xl md:text-5xl font-display font-medium text-primary text-center tracking-tighter italic">
            ...
            <motion.div 
              ...
              className="bg-primary p-10 md:p-14 rounded-none shadow-premium text-center space-y-6"
            >
              <div className="w-16 h-16 rounded-none bg-white/10 mx-auto flex items-center justify-center text-white mb-4 border border-white/20">
                <Check size={32} />
              </div>
              <div className="space-y-4">
                <h2 className="text-4xl md:text-5xl font-display font-medium text-white italic leading-none">Brief <br/> <span className="opacity-40">Received.</span></h2>
                <p className="text-lg text-white/60 font-sans italic leading-relaxed max-w-2xl mx-auto border-t border-white/10 pt-6 md:pt-8">
      ...
      {currentStep > 1 && !isSubmitted && (
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2">
          <button 
            onClick={prevStep}
            className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-primary/40 hover:text-primary transition-colors italic cursor-pointer"
          >
...
function OptionCard({ label, selected, onClick, icon: Icon, multi }: { label: string, selected: boolean, onClick: () => void, icon?: any, multi?: boolean }) {
  return (
    <motion.div
      ...
      onClick={onClick}
      className={`
        relative p-5 md:p-6 rounded-none border-[0.5px] cursor-pointer transition-all duration-500 min-h-[110px] md:min-h-[120px] flex flex-col justify-end group overflow-hidden
        ${selected 
    ...
    >
      ...
      <div className={`
        absolute top-4 right-4 w-6 h-6 rounded-none border-[0.5px] flex items-center justify-center transition-all
        ${selected 
      ...
      <div className="relative z-10 space-y-3 mt-2">
        {Icon && <Icon size={20} className={`transition-colors ${selected ? 'text-primary' : 'text-primary/20 group-hover:text-primary/40'}`} />}
        <h4 className={`text-base md:text-lg font-display italic font-medium leading-tight tracking-tight ${selected ? 'text-primary' : 'text-primary/60 group-hover:text-primary'}`}>
          {label}
        </h4>
      </div>
    </motion.div>
...
function InputBlock({ label, value, onChange, icon: Icon }: { label: string, value: string, onChange: (v: string) => void, icon: any }) {
  return (
    <div className="relative">
      <Icon className="absolute left-5 top-1/2 -translate-y-1/2 text-primary/30" size={18} />
      <input 
        type="text" 
        placeholder={label}
        value={value}
        onChange={e => onChange(e.target.value)}
        className="w-full bg-white border border-outline-variant/10 rounded-none p-4 pl-14 text-sm focus:outline-none focus:border-primary/40 transition-all font-sans"
      />
    </div>
  )
}
